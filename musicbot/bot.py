...
class MusicBot(discord.Client):
  ...
  async def _auto_summon(self):
    ...
    if owner:
      self.safe_print(
        "Found owner in \"%s\", attempting to join..." %
  ...
  async def generate_invite_link(self, *, permissions=None, server=None):
    ...
    return discord.utils.oauth_url(self.cached_client_id,
                                   permissions=permissions, server=server)
  ...
  async def get_voice_client(self, channel):
    ...
    with await self.voice_client_connect_lock:
      ...
      voice_data = await asyncio.wait_for(_voice_data, timeout=10,
                                          loop=self.loop)
      ...
      for x in range(retries):
        try:
          ...
          await asyncio.wait_for(voice_client.connect(), timeout=10,
                                 loop=self.loop)
          ...
          if x == retries - 1:
            raise exceptions.HelpfulError(
              ...
              "It's most likely a system firewall or overbearing anti-virus "
              "firewall.  "
            )
    ...
    if channel and author:
      ...
      if self.server_specific_data[channel.server]['last_np_msg']:
        self.server_specific_data[channel.server][
          'last_np_msg'] = await self.safe_edit_message(last_np_msg, newmsg,
                                                        send_if_fail=True)
      else:
        self.server_specific_data[channel.server][
  ...
  async def safe_delete_message(self, message, *, quiet=False):
    try:
      ...
      if not quiet:
        self.safe_print(
          "Warning: Cannot delete message \"%s\", no permission" %
  ...
  async def on_ready(self):
    ...
    if owner and self.servers:
      ...
      if self.user.bot:
        ...
        print("Note: You should be logged into your main account and have \n"
              "manage server permissions on the server you want the bot to "
              "join.\n")
    ...
    if self.config.bound_channels:
      ...
      [self.safe_print(' - %s/%s' % (ch.server.name.strip(), ch.name.strip()))
    ...
    print("  Skip threshold: %s votes or %s%%" % (
      ...
      self._fixg(self.config.skip_ratio_required * 100)))
    print("  Now Playing @mentions: " + ['Disabled', 'Enabled'][
      self.config.now_playing_mentions])
    print("  Auto-Summon: " + ['Disabled', 'Enabled'][self.config.auto_summon])
    print(
      "  Auto-Playlist: " + ['Disabled', 'Enabled'][self.config.auto_playlist])
    ...
    if self.config.autojoin_channels:
      ...
      if owner_vc:
        print("Done!",
              flush=True)  # TODO: Change this to "Joined server/channel"
        if self.config.auto_playlist:
  ...
  async def cmd_help(self, command=None):
    ...
    if command:
      ...
      helpmsg += "```"
      helpmsg += "https://github.com/SexualRhinoceros/MusicBot/wiki/Commands" \
                 "-list"
  ...
  async def cmd_blacklist(self, message, user_mentions, option, something):
    ...
    if option in ['+', 'add']:
      ...
      if self.blacklist.isdisjoint(user.id for user in user_mentions):
        return Response('none of those users are in the blacklist.', reply=True,
                        delete_after=10)
  ...
  async def cmd_id(self, author, user_mentions):
    ...
    if not user_mentions:
      ...
      usr = user_mentions[0]
      return Response("%s's id is `%s`" % (usr.name, usr.id), reply=True,
                      delete_after=35)
  ...
  async def cmd_play(self, player, channel, author, permissions, leftover_args,
                     song_url):
    """
    Usage:
    ...
    if permissions.max_songs and player.playlist.count_for_user(
        author) >= permissions.max_songs:
      raise exceptions.PermissionsError(
    ...
    try:
      info = await self.downloader.extract_info(player.playlist.loop, song_url,
    ...
    if info.get('url', '').startswith('ytsearch'):
      ...
      if not info:
        raise exceptions.CommandError(
          "Error extracting info from search string, youtubedl returned no "
          "data.  "
    ...
    # TODO: Possibly add another check here to see about things like the
    # bandcamp issue
    ...
    if 'entries' in info:
      ...
      if not permissions.allow_playlists and ':search' in info[
        'extractor'] and len(info['entries']) > 1:
        raise exceptions.PermissionsError(
          "You are not allowed to request playlists", expire_in=30)

      # The only reason we would use this over `len(info['entries'])` is if
      # we add `if _` to this one
      ...
      # other check back in
      if permissions.max_songs and player.playlist.count_for_user(
          author) + num_songs > permissions.max_songs:
        raise exceptions.PermissionsError(
          "Playlist entries + your already queued songs reached limit (%s + "
          "%s > %s)" % (
            num_songs, player.playlist.count_for_user(author),
            permissions.max_songs),
          expire_in=30
        )

      if info['extractor'].lower() in ['youtube:playlist', 'soundcloud:set',
                                       'bandcamp:album']:
        try:
          return await self._cmd_play_playlist_async(player, channel, author,
                                                     permissions, song_url,
                                                     info['extractor'])
      ...
      # My test was 1.2 seconds per song, but we maybe should fudge it a bit,
      #  unless we can
      # monitor it and edit the message with the estimated time, but that's
      # some ADVANCED SHIT
      # I don't think we can hook into it anyways, so this will have to do.
      # It would probably be a thread to check a few playlists and get the
      # speed from that
      ...
      # functions, and every play list might be asyncified
      ...
      if permissions.max_song_length:
        for e in entry_list.copy():
          if e.duration > permissions.max_song_length:
            ...
            # Unless the first entry starts being played, which would make
            # this a race condition
        if drop_count:
          print("Dropped %s songs" % drop_count)

      print(
        "Processed {} songs in {} seconds at {:.2f}s/song, {:+.2g}/song from "
        "expected ({}s)".format(
          listlen,
          self._fixg(ttime),
          ttime / listlen,
      ...
      if permissions.max_song_length and info.get('duration',
                                                  0) > \
          permissions.max_song_length:
        raise exceptions.PermissionsError(
      ...
      try:
        ...
        if e.use_url == song_url:
          print(
            "[Warning] Determined incorrect entry type, but suggested url is "
            "the same.  Help.")

        if self.config.debug_mode:
          print(
            "[Info] Assumed url \"%s\" was a single entry, was actually a "
            "playlist" % song_url)
  ...
  async def _cmd_play_playlist_async(self, player, channel, author, permissions,
                                     playlist_url, extractor_type):
    """
    Secret handler to use the async wizardry to make playlist queuing
    non-"blocking"
    """
    ...
    if permissions.max_song_length:
      ...
      if player.current_entry and player.current_entry.duration > \
        ...
        await self.safe_delete_message(
          self.server_specific_data[channel.server]['last_np_msg'])
        self.server_specific_data[channel.server]['last_np_msg'] = None
        skipped = True
        player.skip()
        entries_added.pop()
    ...
    if not songs_added:
      basetext = "No songs were added, all songs were over max duration (" \
                 "%ss)" % permissions.max_song_length
      if skipped:
        basetext += "\nAdditionally, the current song was skipped for being " \
                    "too long."
  ...
  async def cmd_search(self, player, channel, author, permissions,
                       leftover_args):
    """
    Usage:
    ...
    service = 'youtube'
    items_requested = 3
    max_items = 10  # this can be whatever, but since ytdl uses about 1000,
    ...
    for e in info['entries']:
      ...
      confirm_message = await self.safe_send_message(channel,
                                                     "Is this ok? Type `y`, "
                                                     "`n` or `exit`")
      response_message = await self.wait_for_message(30, author=author,
                                                     channel=channel,
                                                     check=check)
  ...
  async def cmd_np(self, player, channel, server, message):
    ...
    if player.current_entry:
      ...
      song_progress = str(timedelta(seconds=player.progress)).lstrip(
        '0').lstrip(':')
      song_total = str(timedelta(seconds=player.current_entry.duration)).lstrip(
        '0').lstrip(':')
      prog_str = '`[%s/%s]`' % (song_progress, song_total)

      if player.current_entry.meta.get('channel',
                                       False) and player.current_entry.meta.get(
          'author', False):
      ...
      self.server_specific_data[server][
        'last_np_msg'] = await self.safe_send_message(channel, np_text)
  ...
  async def cmd_summon(self, channel, author, voice_channel):
    ...
    if voice_client and voice_client.channel.server == \
        author.voice_channel.server:
  ...
  async def cmd_shuffle(self, channel, player):
    ...
    return Response(":twisted_rightwards_arrows: :ok_hand:",
                    delete_after=15)
  ...
  async def cmd_skipto(self, player, channel, author, message, permissions,
    ...
    try:
      ...
      return Response(
        "enter a number.  NUMBER.  That means digits.  `15`.  Etc.", reply=True,
        delete_after=8)
  ...
  async def cmd_skip(self, player, channel, author, message, permissions,
                     voice_channel):
    """
    Usage:
        {command_prefix}skip
      ...
      if player.playlist.peek():
        if player.playlist.peek()._is_downloading:
          ...
          print("Something odd is happening.  "
                "You might want to restart the bot if it doesn't start "
                "working.")
      else:
    ...
    # TODO: ignore person if they're deaf or take them out of the list or
    # something?
    ...
    skips_remaining = min(self.config.skips_required,
                          sane_round_int(
                            num_voice * self.config.skip_ratio_required)) - \
                      num_skips
  ...
  async def cmd_volume(self, message, player, new_volume=None):
    ...
    Putting + or - before the volume will make the volume change relative to
    the current volume.
    """

    if not new_volume:
      return Response('Current volume: `%s%%`' % int(player.volume * 100),
    ...
    if 0 < new_volume <= 100:
      ...
      return Response('updated volume from %d to %d' % (old_volume, new_volume),
                      reply=True, delete_after=20)

    else:
      if relative:
        raise exceptions.CommandError(
          'Unreasonable volume change provided: {}{:+} -> {}%.  Provide a '
          'change between {} and {:+}.'.format(
            old_volume, vol_change, old_volume + vol_change, 1 - old_volume,
                                    100 - old_volume), expire_in=20)
      else:
        raise exceptions.CommandError(
          'Unreasonable volume provided: {}%. Provide a value between 1 and '
          '100.'.format(
  ...
  async def cmd_queue(self, channel, player):
    ...
    for i, item in enumerate(player.playlist, 1):
      if item.meta.get('channel', False) and item.meta.get('author', False):
        nextline = '`{}.` **{}** added by **{}**'.format(i, item.title,
                                                         item.meta[
                                                           'author'].name).strip()
      else:
      ...
      if currentlinesum + len(nextline) + len(
          andmoretext) > DISCORD_MSG_CHAR_LIMIT:
    ...
    if not lines:
      ...
      repeat_status = ""
      if self.config.repeat:
        repeat_status += ':repeat_one: **on**'
      else:
        repeat_status += ':repeat_one: **off**'
      repeat_status += " / "
      if self.config.repeat_all:
        repeat_status += ':repeat: **on**'
      else:
        repeat_status += ':repeat: **off**'
  ...
  async def cmd_clean(self, message, channel, server, author, search_range=50):
    ...
    Removes up to [range] messages the bot has posted in chat. Default: 50,
    Max: 1000
    """
    ...
    if self.user.bot:
      if channel.permissions_for(server.me).manage_messages:
        deleted = await self.purge_from(channel, check=check,
  ...
  async def cmd_pldump(self, channel, song_url):
    ...
    try:
      info = await self.downloader.extract_info(self.loop, song_url.strip('<>'),
                                                download=False, process=False)
    except Exception as e:
      raise exceptions.CommandError(
  ...
  async def cmd_listids(self, server, author, leftover_args, cat='all'):
    ...
    for cur_cat in requested_cats:
      ...
      if cur_cat == 'users':
        data.append("\nUser IDs:")
        rawudata = ['%s #%s: %s' % (m.name, m.discriminator, m.id) for m in
                    server.members]
        ...
        data.append("\nText Channel IDs:")
        tchans = [c for c in server.channels if
                  c.type == discord.ChannelType.text]
        rawudata = ['%s: %s' % (c.name, c.id) for c in tchans]

        rawudata.append("\nVoice Channel IDs:")
        vchans = [c for c in server.channels if
  ...
  async def cmd_perms(self, author, channel, server, permissions):
    ...
    for perm in permissions.__dict__:
      ...
      lines.insert(len(lines) - 1,
                   "%s: %s" % (perm, permissions.__dict__[perm]))
  ...
  async def cmd_setavatar(self, message, url=None):
    ...
    try:
      ...
      raise exceptions.CommandError("Unable to change avatar: %s" % e,
                                    expire_in=20)

    return Response(":ok_hand:", delete_after=20)

  async def cmd_dismiss(self, server):
  ...
  async def on_message(self, message):
    ...
    command, *args = message_content.split()  # Uh, doesn't this break
    # prefixes with spaces in them (it doesn't, config parser already breaks
    # them)
    ...
    if message.channel.is_private:
      if not (
          message.author.id == self.config.owner_id and command ==
              'joinserver'):
        await self.send_message(message.channel,
                                'You cannot use this bot in private messages.')
        return

    if message.author.id in self.blacklist and message.author.id != \
        self.config.owner_id:
      self.safe_print(
        "[User blacklisted] {0.id}/{0.name} ({1})".format(message.author,
                                                          message_content))
      return
    ...
    try:
      ...
      for key, param in list(params.items()):
        doc_key = '[%s=%s]' % (key,
                               param.default) if param.default is not \
      ...
      if message.author.id != self.config.owner_id:
        if user_permissions.command_whitelist and command not in \
          ...
          raise exceptions.PermissionsError(
            "This command is not enabled for your group (%s)." %
          ...
          raise exceptions.PermissionsError(
            "This command is disabled for your group (%s)." %
            user_permissions.name,
            expire_in=20)
  ...
  async def on_voice_state_update(self, before, after):
    ...
    my_voice_channel = after.server.me.voice_channel  # This should always
    # work, right?
  ...
  async def on_server_update(self, before: discord.Server,
