...
class Playlist(EventEmitter):
  ...
  async def add_entry(self, song_url, **meta):
    ...
    try:
      info = await self.downloader.extract_info(self.loop, song_url,
                                                download=False)
    ...
    if info.get('_type', None) == 'playlist':
      raise WrongEntryTypeError("This is a playlist.", True,
                                info.get('webpage_url', None) or info.get('url',
                                                                          None))
    ...
    if info['extractor'] in ['generic', 'Dropbox']:
      try:
        ...
        content_type = await get_header(self.bot.aiosession, info['url'],
                                        'CONTENT-TYPE')
      ...
      if content_type:
        if content_type.startswith(('application/', 'image/')):
          if '/ogg' not in content_type:  # How does a server say
              # `application/ogg` what the actual fuck
            raise ExtractionError("Invalid content type \"%s\" for url %s" % (
  ...
  async def import_from(self, playlist_url, **meta):
    ...
    try:
      info = await self.downloader.safe_extract_info(self.loop, playlist_url,
                                                     download=False)
    except Exception as e:
      raise ExtractionError(
    ...
    for items in info['entries']:
      if items:
        try:
          ...
          baditems += 1
          # Once I know more about what's happening here I can add a proper
          # message
          traceback.print_exc()
          print(items)
  ...
  async def get_next_entry(self, repeat_all):
    """
        A coroutine which will return the next song or None if no songs left
        to play.
    """
    if not self.entries:
      return None
    ...
    # If repeating all, re-add the entry to the end of the playlist.
    if repeat_all:
      self.entries.append(entry)

    # Pre-download the next song
