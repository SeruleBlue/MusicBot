...
class PatchedBuff:
  """
      PatchedBuff monkey patches a readable object, allowing you to vary what
      the volume is as the song is playing.
  """
...
class MusicPlayerState(Enum):
  ...
  WAITING = 3  # The player has finished its song but is still downloading
  # the next one
...
class MusicPlayer(EventEmitter):
  ...
  def _playback_finished(self):
    ...
    if not self.bot.config.save_videos and entry:
      if any([entry.filename == e.filename for e in self.playlist.entries]):
        ...
        # print("[Config:SaveVideos] Deleting file: %s" % os.path.relpath(
        # entry.filename))
  ...
  async def _delete_file(self, filename):
    for x in range(30):
      ...
      print(
        "[Config:SaveVideos] Could not delete file {}, giving up and moving "
        "on".format(
  ...
  async def _play(self, _continue=False):
    """
        Plays the next entry from the playlist, or resumes playback of the
        current entry if paused.
    """
    if self.is_paused:
      return self.resume()
  ...
  async def websocket_check(self):
    ...
    while not self.is_dead:
      try:
        ...
        if self.bot.config.debug_mode:
          print(
            "[Debug] Voice websocket is %s, reconnecting" %
...
# I don't know if we can even do this for the user, at most we open it in the
#  browser
# I can't imagine the user is so incompetent that they can't pull 3 files out
#  of it...
# ...
# ...right?
...
# default=noprint_wrappers=1:nokey=1 -sexagesimal filename.mp3
